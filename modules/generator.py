...
class SPADE_layer(nn.Module):
    ...
    def forward(self, x, modulation_in):
        if is_in_onnx_export():
            # Use custom instance norm symbolic function during ONNX export
            normalized = self.custom_instance_norm(x)
        else:
            # Normal PyTorch instance normalization during normal execution
    ...
    def custom_instance_norm(self, x):
        # Manually perform instance normalization
        mean = x.mean([2, 3], keepdim=True)
        var = x.var([2, 3], keepdim=True, unbiased=False)
        eps = 1e-5  # Use a small epsilon for numerical stability
